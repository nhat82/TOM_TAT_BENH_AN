"""Masking of personally identifiable columns returned by SQL tools.

Flow:
  mask_rows()    DB rows -> placeholders, applied before data reaches the LLM.
                 Each distinct raw value is registered in the request's vault
                 as "[ho_ten_1]", "[ho_ten_2]", ... so different patients in the
                 same result set never collapse onto the same placeholder.
  unmask_text()  LLM answer -> real values, applied at the API boundary only,
                 resolved from that same vault. No extra DB query.

The vault is per conversation thread (same key as the agent checkpointer), so
placeholders stay stable across turns and text coming back from the client can
be re-masked before it re-enters the model context.
"""
