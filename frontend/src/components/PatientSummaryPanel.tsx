  ...
  return (
    <>
      <div className="bg-white border border-outline-variant rounded-[8px] overflow-hidden shadow-card">
        ...
        {!collapsed && <div className="p-[18px] flex flex-col gap-[10px]">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-primary text-white rounded-[6px] text-[14px] font-semibold cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed transition-opacity"
          >
            {isGenerating ? (
          ...
          {error && (
            <p className="text-xs text-[#b5544b] bg-[#fdf0ee] border border-[#f3d9d4] rounded-md px-3 py-2">{error}</p>
          )}

          {/* Version control */}
          {(hasSummary || isWorking) && (
            <div className="flex flex-col items-center gap-[6px] bg-[#f6f9fe] border border-[#e9eff8] rounded-[5px] px-[9px] py-[10px]">
              <div className="flex items-center gap-[7px]">
                <button
                  onClick={() => setVIndex(i => Math.max(0, i - 1))}
                  disabled={vIndex <= 0}
                  className="w-[26px] h-[26px] inline-flex items-center justify-center border border-outline-variant rounded-[4px] bg-white disabled:text-[#c8d1de] text-[#3c5573] disabled:cursor-default cursor-pointer"
                >
          ...
          {(() => {
            ...
            if (parsed) {
              return (
                <div className="w-full border border-outline-variant rounded-[6px] bg-surface-container-low p-3 flex flex-col gap-[10px] min-h-[184px] overflow-y-auto max-h-[400px]">
            ...
            return (
              <textarea
                ...
                className="w-full h-[184px] p-3 border border-outline-variant rounded-[6px] text-[13px] leading-[1.55] text-on-surface resize-none outline-none font-[inherit] bg-surface-container-low focus:border-primary transition-colors"
              />
            )
          })()}
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="w-[28px] h-[28px] inline-flex items-center justify-center rounded-md border transition-colors"
                  style={{
              ...
              {showReason && !feedbackSent && (
                <div className="mt-2 bg-[#fdf3f1] border border-[#f3d9d4] rounded-[6px] p-[10px] animate-[fadeUp_.2s_ease]">
                  ...
                  <textarea
                    ...
                    className="w-full h-[54px] text-[12px] border border-[#f0d3cd] rounded-md p-2 resize-none outline-none font-[inherit] bg-white text-on-surface"
                  />
                  <button
                    onClick={() => { setFeedbackSent(true); setShowReason(false) }}
                    className="mt-[6px] px-[14px] py-[6px] bg-[#d8584f] text-white text-[12px] font-semibold rounded-md border-none cursor-pointer font-[inherit]"
                  >
                    Gửi phản hồi
                  </button>
                </div>
          ...
          <div className="flex gap-[10px] mt-1">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-[7px] py-[10px] border border-[#cfdbec] rounded-[5px] bg-white text-[#3c5573] text-[13px] font-semibold cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:bg-surface-container-low transition-colors font-[inherit]"
            >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-[7px] py-[10px] border-none rounded-[5px] bg-primary-container text-primary text-[13px] font-semibold cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity font-[inherit]"
            >
      ...
      {previewHtml && createPortal(
        ...
        >
          <div
            className="bg-white rounded-md shadow-2xl w-full max-w-4xl h-[96vh] flex flex-col"
          ...
          >
            ...
            <div className="flex items-center gap-sm px-6 py-3 border-t border-outline-variant shrink-0">
              <button
                onClick={() => setPreviewHtml(null)}
                className="ml-auto px-4 py-2 bg-primary text-white rounded-md text-xs font-bold hover:opacity-90 transition-opacity uppercase tracking-wider"
              >
                Đóng
              </button>
            </div>
